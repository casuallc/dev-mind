...
- CAP-51 需求粒度工作区依赖 CAP-42/25/31/34/41（并取代 CAP-42 FR-01~06/09）：launch 帧 workspaceKey（协议 v10 门控）把工作区落到 `<owner>/worktrees/<req-<需求id>|sid-<会话id>>`，分支 `feature/req-<需求id>`，同需求会话串行复用、跨需求并行；收口上移需求级（合并基线+push+保留工作树），工作树纳入对账与超龄 GC（`worktreeGcDays`），存量 `workspace_key IS NULL` 会话按旧布局兼容收口/释放。
- CAP-54 工作区实时视图依赖 CAP-21/30/34/42/51/53：runner 侧采集以 CAP-53 代码目录（`worktrees/<key>`）为基准，触发器挂 common 会话内核的 CLI 事件出口（tool_result）；服务端经 `WorkspaceStatusListener` SPI（session/chat 各自实现）把快照旁路推给浏览器 WS，文件树/内容/diff 经 workspace_query 请求应答帧（协议 v11 门控）透传 runner 读盘，零表结构变更、纯只读不改工作区生命周期。
