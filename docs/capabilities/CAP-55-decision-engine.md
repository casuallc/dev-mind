...
- **HTTP 常驻进程，不走 runner exec 帧（已定）**：laya 冷启动 7-10s（CPU 7.4s / T4 10.3s，官方实测），
  每次起进程式调用让 33ms 决策变成 10s 级，模型必须常驻内存；exec 帧是分钟级任务语义（日志流/状态机），
  套 100ms 级同步 RPC 等于重造协议，且 agent WS 每连接串行派发、同步等答有自死锁前科。
  部署位置（可落 runner 节点机）与调用协议（HTTP）是两件事。**批量负载例外**：全量重分诊、
  训练数据批量打分、微调训练等离线场景冷启动可摊薄，走 runner exec 帧 + 日志 Hub 正合适（后续 CAP）。
