 ...
 *
 * <p>{@code batchSize}/{@code topK}/{@code threshold} 是<b>向量语义</b>（FR-11）：{@code kind=CHAT}
 * 时服务端把它们一律落 null（越界值也不报错），调用方不必为此分叉。</p>
 *
 * @param kind           EMBEDDING | CHAT（RERANK 预留，传值 400）
 * @param name           展示名（创建必填）
